...
function Snake(props) {
  ...
  const createBody = () => {
    ...
    for (let i = 0; i < snake.length; ++i) {
      if (i === 0)
        body.push(<SnakeHead position={{gridArea: `${snake[i].y} / ${snake[i].x} / span 1 / span 1`}} key={`snake${i}`}/>);
      else if (i === bodyLength - 1)
        body.push(<SnakeTail position={{gridArea: `${snake[i].y} / ${snake[i].x} / span 1 / span 1`}} key={`snake${i}`}/>);
      else
        body.push(<SnakeBody position={{gridArea: `${snake[i].y} / ${snake[i].x} / span 1 / span 1`}} key={`snake${i}`}/>);
    }
    return body;
  }
